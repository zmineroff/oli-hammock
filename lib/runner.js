...
class Runner {
     ...
     */
    constructor(superActivity, activity, question) {
    ...
    render() {
        return this.stored.then(({ state, feedback }) => {
            const questionData = {
                ...
                state: state,
                parts: this.question.parts.map((part, i) => {
                    return {
                        prompt: part.prompt,
    ...
    grade(state, config) {
        return this.activity.parse(state, config).map((response, i) => {
            ...
            if (match) {
                return {
                    key: view.key,
                    status: match.score >= this.question.parts[i].score ? activity_1.STATUS_CORRECT : activity_1.STATUS_INCORRECT,
            ...
            return {
                key: view.key,
                status: activity_1.STATUS_NOT_FOUND,
                score: 0,
                message: view.key
            };
        });
    ...
    submit() {
        ...
        return this.stored.then(({ state, feedback }) => {
            ...
            // Part logging
            const dashboardLogging = feedback.map((value, part) => {
                if (value === null)
                    return Promise.resolve();
                const step = `p${part + 1}`;
                return new Promise(resolve => {
                    const action = new ActionLog("SUBMIT_ATTEMPT", this.superActivity.sessionId, this.superActivity.resourceId, this.superActivity.activityGuid, "OLI_HAMMOCK_ACTIVITY", this.superActivity.timeZone);
